...
function App() {
  ...
  const shuffle = (e) => {
    ...
    setDeck(mazzo.map((item) => item.number + item.suit));

  };


  const [deck, setDeck] = useState(mazzo.map((item) => item.number + item.suit));
  ...
  return (
    <div className='d-flex justify-content-center align-items-center flex-column my-5 p-3'>
      ...
      <span className='log-mazzo2 m-1 nes-text is-primary d-flex'>{deck}</span>
